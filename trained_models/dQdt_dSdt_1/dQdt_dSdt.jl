...
#         println("s_new ", maximum(s_new))
        ...
        if isapprox(i % t_ratio, 0.0, atol=1e-6)
            ...
            # cumulate the loss
            # sum(abs2, S0 - atrain1[:,:,index])
            # sum(abs2, u0 - ytrain21[:, :, index]) + sum(abs2, ∂x1 * u0 - ∂x1 * ytrain21[:, :, index])
            l = sum(abs2, ∂x1 * u0 - ∂x1 * ytrain21[:, :, index])
            loss += l
        end
...
for j in 1:n_epochs
    ...
    if j % 3 == 0 && learning_rate > 1e-6
        ...
        for i in 1:test_batch_iterations
			...
			println("MSE loss S - interpolation", sum(abs2,pred_S_matrix-atest))
        ...
        for i in 1:ex_batch_iterations
			...
			println("MSE loss S - extrapolation", sum(abs2,pred_S_matrix-aex))
